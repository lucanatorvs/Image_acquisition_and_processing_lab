...
bool imgSave(Mat img, string fname);
...
int ShowFPS = false;
volatile int ExposureTime = 200;      //FIX EMEEEE!!!!!
...
// ****************************************************************************
    ...
    cfg.exposureMS = (float)ExposureTime;
...
// check for 'Esc' (or 'backspace' or 'enter') to stop
	if ((key == 0x1b) || (key == 0x08) || (key == 0x0d)) {
	    ...
	    case '?':
		cout << "ROI width = " << image.cols << ", height = " << image.rows << endl;
		break;

	    case 'e':
		cout << "Exposure time set to: " << cfg.exposureMS << " ms" << endl;
		break;
...
// ***************************************************************************
    ...
    return imwrite(fname, img, params);
}
///* EOF hhs_cam.cpp  */
